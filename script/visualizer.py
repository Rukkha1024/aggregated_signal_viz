...
class AggregatedSignalVisualizer:
    ...
    def _run_mode(self, signal_group: str, mode_name: str, mode_cfg: Dict) -> None:
        ...
        if overlay:
            ...
            sorted_keys = self._sort_overlay_keys(list(aggregated_by_key.keys()), group_fields)
    ...
    def _sort_overlay_keys(self, keys: List[Tuple], group_fields: List[str]) -> List[Tuple]:
    ...
    def _format_group_label(self, key: Tuple, group_fields: List[str]) -> str:
    ...
    def _plot_emg_overlay(
        ...
        for ax, ch in zip(axes_flat, channels):
            ...
            for key in sorted_keys:
                ...
                if y is None:
                    continue
                group_label = self._format_group_label(key, group_fields)
                ax.plot(
                    x,
                    y,
                ...
                if self.common_style.get("show_onset_marker", True):
                    ...
                    if onset_time is not None and self._is_within_time_axis(onset_time):
                        ...
                        if onset_norm is not None:
                            ax.axvline(onset_norm, **self.emg_style["onset_marker"], label=f"{group_label} onset")
                if self.common_style.get("show_max_marker", True):
                    ...
                    if max_time is not None and self._is_within_time_axis(max_time):
                        ...
                        if max_norm is not None:
                            ax.axvline(max_norm, **self.emg_style["max_marker"], label=f"{group_label} max")
    ...
    def _plot_forceplate_overlay(
        ...
        fig, axes = plt.subplots(rows, cols, figsize=self.forceplate_style["subplot_size"], dpi=self.common_style["dpi"])
        x = self.x_norm if self.x_norm is not None else self.target_axis
        ...
        for ax, ch in zip(np.ravel(axes), self.config["signal_groups"]["forceplate"]["columns"]):
            ...
            if self.common_style.get("show_onset_marker", True):
                for key in sorted_keys:
                    ...
                    if onset_time is None or not self._is_within_time_axis(onset_time):
                        continue
                    onset_norm = self._ms_to_norm(onset_time)
                    if onset_norm is None:
                        continue
                    group_label = self._format_group_label(key, group_fields)
                    ax.axvline(onset_norm, **self.forceplate_style["onset_marker"], label=f"{group_label} onset")
